...
先程のコードの`nega_max_next_action`で、1つ目の`action`のスコアが「0」（引き分け）と出た場合を考えてください。2つ目以降の`action`で選ばれる可能性があるのは、スコアが0を超える場合、スコアが「1」（勝ち）の場合だけですよね？　もし2つめの`action`のスコアが「-1」（負け）なら、それを選ぶのは馬鹿げています。そして「0」なら、わざわざ手を変える必要はありません。
